...
const CONFIG = {
	...
	get versionUrl() {
		return `http://${this.host}:${this.port}/api/version`
	}
}
...
function getFlagValue(flag: string): string | undefined {
	...
	const eqArg = process.argv.find(function (a) {
		return a.startsWith(fullFlag + '=')
	})
...
function pullModel(modelName: string) {
	...
	if (CONFIG.host === 'localhost' || CONFIG.host === '127.0.0.1') {
		...
		log(
			`Cannot pull model automatically on remote host. Please run 'ollama pull ${modelName}' on ${CONFIG.host}.`,
			colors.yellow
		)
		process.exit(1)
	}
...
async function verifyModel(modelName: string) {
	...
	try {
		...
		if (e.cause?.code === 'ECONNREFUSED') {
			...
			log(
				'3. Ensure OLLAMA_HOST and OLLAMA_ORIGINS are configured to allow connections.',
				colors.yellow
			)
		}
		process.exit(1)
