...
async function reset() {
	...
	if (!args.includes('--force')) {
		...
		const answer = await new Promise<string>((resolve) => {
			rl.question(
				`\n${colors.bold}Are you sure you want to proceed? (yes/N)${colors.reset} `,
				(result) => {
					rl.close()
					resolve(result)
				}
...
function introspect() {
	...
	try {
		const tables = execSync(
			`sqlite3 "${dbPath}" "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%';"`
		)
			.toString()
			.split('\n')
			.filter(Boolean)
