...
`Database::Aurora`, which is absurd. They are the **same Postgres engine with a label**.
...
`DatabaseType` (adapter engine id) is `Postgres | MySQL | SQLite | DuckDB | LibSQL`.
...
`PgDialect` / `MySqlDialect` (in `dialect.rs`) use enum-as-strategy (closed, vendor-controlled
...
Adapters (`PostgresAdapter`/`MySqlAdapter`) carry the dialect field, mirroring the existing
