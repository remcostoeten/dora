...
import { CodeEditor } from '@studio/features/drizzle-runner/components/code-editor'
...
function isTextEntryTarget(event: KeyboardEvent): boolean {
	...
	return candidates.some(function (candidate) {
		...
		return (
			tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT' || candidate.isContentEditable
		)
	})
}
...
function SqlConsoleInner({ isActive = true, activeConnectionId, getConnectionName }: Props) {
					...
					<div className='space-y-3'>
						{['w-5/12', 'w-8/12', 'w-6/12', 'w-7/12', 'w-4/12'].map(
							function (width, index) {
								return (
									<Skeleton key={index} className={`h-4 bg-white/10 ${width}`} />
								)
							}
						)}
					</div>
	...
	useActiveScope($, 'sql-console', isActive)

	$.bind(sqlShortcuts.runQuery.combo)
		.in('sql-console')
		.on(
			function () {
				handleExecute()
	...
	$.bind(sqlShortcuts.runSelection.combo)
		.in('sql-console')
		.on(
			function () {
	...
	$.bind(sqlShortcuts.openQueryHistory.combo)
		.in('sql-console')
		.on(
			function () {
	...
	$.bind(sqlShortcuts.aiCmdK.combo)
		.in('sql-console')
		.on(
			function () {
				if (hideAi) return
	...
	$.bind(sqlShortcuts.switchToDrizzle.combo)
		.in('sql-console')
		.on(
			function () {
				setMode('drizzle')
	...
	$.bind(sqlShortcuts.switchToPrisma.combo)
		.in('sql-console')
		.on(
			function () {
				setMode('prisma')
												...
												<Suspense fallback={renderEditorFallback()}>
													{mode === 'sql' ? (
														<SqlEditor
															...
															onSave={
																handleSaveActiveSnippetFromEditor
															}
															onModeChange={setMode}
				...
				<div
					...
					onMouseDown={(e) => {
						...
						function onMove(ev: MouseEvent) {
							...
							setSidebarWidth(sbwRef.current)
							setShowRightSidebar(
								getSqlConsoleSidebarStateForWidth(sbwRef.current).isOpen
							)
						}
						function onUp() {
