...
import { Dialog, DialogContent, DialogDescription, DialogTitle } from "@studio/shared/ui/dialog";
import { Input } from "@studio/shared/ui/input";
import { ScrollArea } from "@studio/shared/ui/scroll-area";
import { Separator } from "@studio/shared/ui/separator";
import { toast } from "@studio/shared/ui/notifier";
import { getTableRefId, getTableSqlIdentifier } from "@studio/shared/utils/table-ref";
...
export function CommandPalette({
        ...
        try {
          ...
          if (scriptsRes.ok) {
            scriptsRes.data.forEach(function (script) {
              const folderId = (script as typeof script & { folder_id?: number | null }).folder_id;
  ...
  const executeItem = useCallback(async function (item: PaletteItem | null) {
  ...
  const breadcrumbLabels = stack.map(function (route) {
    switch (route.kind) {
      ...
      case "snippet":
        return (
          snippets.find(function (snippet) {
