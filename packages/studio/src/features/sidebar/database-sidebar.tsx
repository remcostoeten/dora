...
import { useSidebarWidth, SIDEBAR_MIN_WIDTH, SIDEBAR_MAX_WIDTH } from './hooks/use-sidebar-width'
...
import { prefetchTableSnapshot } from '../database-studio/utils/table-snapshot'
...
export function DatabaseSidebar({
	...
	useEffect(
		function handleAutoSelectFirstTable() {
			if (!autoSelectFirstTable || !schema || schema.tables.length === 0 || !onTableSelect)
				return
	...
	const handleTablePrefetch = useCallback(
		function (tableId: string) {
			void prefetchTableSnapshot(adapter, activeConnectionId, tableId)
		},
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							{getTableRefParts(targetTableName).tableName}&quot;? This action cannot
							be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
