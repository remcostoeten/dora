...
function isDrizzleModel(model: Monaco.editor.ITextModel): boolean {
	...
	return uri.includes('/typescript/drizzle%3A') || uri.includes('/typescript/drizzle-runner%3A')
}
...
export function CodeEditor({
	...
	const handleEditorDidMount: EditorMount = function (editor, monaco) {
						...
						if (table) {
							return buildSuggestions(range, [
								...
								},
								{
									label: 'columns',
									kind: monaco.languages.CompletionItemKind.Property,
									insertText: `columns: ${valuesSnippet(table, true).replace(/:\s*[^,}]+/g, ': true')},`,
									insertTextRules:
										monaco.languages.CompletionItemInsertTextRule
											.InsertAsSnippet,
		...
		signatureProviderRef.current = monaco.languages.registerSignatureHelpProvider(
			'typescript',
			{
