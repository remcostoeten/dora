...
async startQueryStream(connectionId: string, query: string, onEvent: TAURI_CHANNEL<QueryEvent>) : Promise<Result<number[], { kind: string; detail: string }>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("start_query_stream", { connectionId, query, onEvent }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async aiResolveProviderModel(provider: string) : Promise<Result<string, { kind: string; detail: string }>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("ai_resolve_provider_model", { provider }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type BootstrapData = { connections: ConnectionInfo[]; 
/**
 ...
 */
settings: string | null; savedQueries: SavedQuery[]; snippets: SavedQuery[]; snippetFolders: SnippetFolder[]; 
/**
...
export type QueryEvent = { type: "columns"; query_id: number; columns: JsonValue } | { type: "row_batch"; query_id: number; page_index: number; rows: JsonValue } | { type: "progress"; query_id: number; rows_received: number } | { type: "done"; query_id: number; affected_rows: number; elapsed_ms: number } | { type: "error"; query_id: number; message: string }
