...
function IndexInner() {
	...
	>()
	const [settingsHighlightSection, setSettingsHighlightSection] = useState<
		SettingsSectionId | undefined
	>()
	...
	inputShortcut.bind(shortcuts.openCommandPalette.combo).on(
		function () {
	...
	const handleConnectionDialogOpenChange = useCallback(function (open: boolean) {
			...
			<SidebarProvider>
				<div className='flex flex-col h-full w-full bg-background overflow-hidden'>
					<div className='flex flex-1 overflow-hidden'>
						...
						<div
							ref={(node) => {
								if (node) node.inert = !showDatabasePanel || !isSidebarOpen
							}}
							className={showDatabasePanel && isSidebarOpen ? 'contents' : 'hidden'}
							hidden={!showDatabasePanel || !isSidebarOpen}
							aria-hidden={!showDatabasePanel || !isSidebarOpen}
						>
							<DatabaseSidebar
								activeNavId={activeNavId}
								onNavSelect={setActiveNavId}
										...
										{connections.length === 0 && !isLoading ? (
											...
											<div className='flex flex-col flex-1 min-h-0'>
												...
												<TabBar
													...
													rightSlot={
														showConnectionTabBar ? undefined : (
															<WindowControls />
														)
													}
												/>
											...
											{isPosthogConnection && activeConnectionId ? (
												...
												<div className='flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center'>
													<p className='text-sm text-muted-foreground'>
														Analytics is available for PostHog
														connections.
													</p>
													<button
														onClick={function () {
										...
										<ErrorBoundary feature='Docker Manager'>
											<DockerView
												...
												onOpenInDataViewer={async function (container) {
													const details =
														getContainerConnectionDetails(container)
						...
						{!settings.hideAi && (
							<AiAssistantPanelHost
								...
								editorContext={
									activeNavId === 'sql-console' ? sqlConsoleEditorContext : null
								}
...
function WorkspaceStartScreenWithTabs({
	shortcut,
	canDropFiles,
	onAddConnection
}: {
