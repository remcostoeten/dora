...
Both `.claude/` and `.agents/` are gitignored here (`.gitignore` lines 38 and 36), so
anything placed under them is local-only and will not reach other clones or CI.
`.codex/` does not exist in this repo and is not currently gitignored.
`.agent/` (singular, no `s`) is a **different, tracked** directory holding
`AGENTS.md`, `claude.md`, `rules/`, and `workflows/` — it is not a skills root for
either tool. `.agent-skills/` (this report's location) is not gitignored.
