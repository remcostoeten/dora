...
| --- | --- |
| Live monitoring | `SourceCaps.supports_live_monitor` is the backend admission gate. Postgres, CockroachDB, MySQL, MariaDB and SQLite are enabled; DuckDB, libSQL, D1 and PostHog are rejected before a monitor task starts, matching the Studio capability table. Push via `LISTEN`/`NOTIFY` is used only where `supports_listen_notify` is true (vanilla Postgres); the other enabled engines poll. |
