...
impl Storage {
    ...
    pub fn create_snippet_folder(&self, name: &str, parent_id: Option<i64>, color: Option<&str>) -> Result<i64> {
        let now = chrono::Utc::now().timestamp();
        let conn = self.conn.lock().unwrap();
    ...
    pub fn update_snippet_folder(&self, id: i64, name: &str, color: Option<&str>) -> Result<()> {
        let now = chrono::Utc::now().timestamp();
        let conn = self.conn.lock().unwrap();
    ...
    pub fn delete_snippet_folder(&self, id: i64) -> Result<()> {
        ...
        // Then delete the folder (cascade will handle child folders)
