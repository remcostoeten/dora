                                            ...
                                            <ContextMenu key={`${table.name}-${col.name}`}>
                                                <ContextMenuTrigger>
                                                    ...
                                                    >
                                                        ...
                                                        <span className={cn(
                                                            "truncate",
                                                            col.primaryKey && "font-medium text-foreground"
                                                        )}>
                                                            {col.name}
                                                        </span>
                                                ...
                                                <ContextMenuContent>
                                                    <ContextMenuItem onClick={() => onInsertQuery?.(col.name)}>
                                                        <FileText className="h-3.5 w-3.5" />
                                                        Insert column name
                                                    </ContextMenuItem>
                                                    <ContextMenuItem onClick={() => onInsertQuery?.(`SELECT ${col.name} FROM ${table.name} LIMIT 100;`)}>
                                                        <Database className="h-3.5 w-3.5" />
                                                        SELECT {col.name}
                                                    </ContextMenuItem>
                                                    <ContextMenuSeparator />
                                                    <ContextMenuItem onClick={() => copyToClipboard(col.name)}>
                                                        <Copy className="h-3.5 w-3.5" />
                                                        Copy column name
                                                    </ContextMenuItem>
                                                    <ContextMenuItem onClick={() => copyToClipboard(col.type)}>
                                                        <Copy className="h-3.5 w-3.5" />
                                                        Copy type: {col.type}
                                                    </ContextMenuItem>
                                                </ContextMenuContent>
