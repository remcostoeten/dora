...
import { getTableData, updateCell, deleteRow } from "./data";
import {
    TableData,
    PaginationState,
    ViewMode,
...
export function DatabaseStudio({ tableId, tableName, onToggleSidebar }: Props) {
    ...
    const handleCellEdit = async (rowIndex: number, columnName: string, newValue: unknown) => {
        ...
        const success = updateCell(tableId, rowIndex, columnName, newValue);
        if (success) {
            await loadTableData();
        } else {
    ...
    const handleRowAction = async (action: string, row: Record<string, unknown>, rowIndex: number) => {
        ...
        switch (action) {
            case "delete":
                const deleteSuccess = deleteRow(tableId, rowIndex);
                if (deleteSuccess) {
                    await loadTableData();
                } else {
