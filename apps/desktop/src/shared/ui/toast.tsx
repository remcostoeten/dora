...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Close
		ref={ref}
		className={cn(
			'absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity group-hover:opacity-100 group-[.destructive]:text-destructive-foreground/80 hover:text-foreground hover:group-[.destructive]:text-destructive-foreground focus:opacity-100 focus:outline-hidden focus:ring-2 focus:group-[.destructive]:ring-destructive focus:group-[.destructive]:ring-offset-destructive',
			className
		)}
		toast-close=''
		{...props}
