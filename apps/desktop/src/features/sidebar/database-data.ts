...
const generateCustomers = (count: number): Record<string, unknown>[] => {
  ...
  const countries = ["USA", "Canada", "UK", "Germany", "France", "Australia", "Japan", "Spain", "Italy", "Netherlands"];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      created_at: new Date(2020 + Math.floor(Math.random() * 5), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)).toISOString(),
      last_login: new Date(2024, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)).toISOString(),
    };
  });
};

const generateOrders = (count: number, customerCount: number): Record<string, unknown>[] => {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      shipped_date: status === "shipped" || status === "delivered" ? new Date(orderDate.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() : null,
      delivered_date: status === "delivered" ? new Date(orderDate.getTime() + (3 + Math.random() * 7) * 24 * 60 * 60 * 1000).toISOString() : null,
    };
  });
};
...
const generateProducts = (count: number): Record<string, unknown>[] => {
  ...
  const products = [
    ...
    "Running Shoes", "Yoga Mat", "Water Bottle", "Backpack", "T-Shirt",
    "Desk Lamp", "Plant Pot", "Wall Art", "Throw Pillow", "Coffee Maker",
    "Tennis Racket", "Basketball", "Soccer Ball", "Dumbbells", "Resistance Bands",
    "Fiction Novel", "Self-Help Book", "Cookbook", "Biography", "Science Fiction",
    "Building Blocks", "Board Game", "Stuffed Animal", "Art Kit", "Puzzle",
    "Coffee Beans", "Tea Set", "Chocolate Box", "Snack Pack", "Energy Bars",
    "Face Cream", "Shampoo", "Lipstick", "Perfume", "Hair Brush",
  ];
...
const generatePageViews = (count: number, userCount: number): Record<string, unknown>[] => {
  const pages = [
    "/home", "/products", "/products/1", "/products/2", "/cart",
    "/checkout", "/about", "/contact", "/blog", "/blog/post-1",
    "/categories", "/search", "/account", "/account/orders", "/account/settings"
  ];
  const referrers = ["https://google.com", "https://facebook.com", "https://twitter.com", null, "https://linkedin.com", "https://direct"];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      page_title: pages[Math.floor(Math.random() * pages.length)].replace('/', '').toUpperCase() || "HOME",
...
const generateAuditLogs = (count: number, userCount: number): Record<string, unknown>[] => {
  const actions = ["login", "logout", "update_profile", "change_password", "create_order", "delete_item", "export_data", "view_dashboard"];
  const entityTypes = ["user", "order", "product", "category", "settings"];
  const userAgents = ["Mozilla/5.0 (Windows NT 10.0; Win64; x64)", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)", "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X)"];
...
const generateArticles = (count: number, authorCount: number): Record<string, unknown>[] => {
  const titles = [
    "The Future of Web Development", "10 Tips for Better Productivity", "Understanding Modern Frameworks",
    "A Guide to Remote Work", "The Psychology of Design", "Best Practices for API Design",
    "Introduction to Machine Learning", "The Art of Debugging", "Cloud Computing Essentials",
    "Mobile Development Trends", "Security Best Practices", "Data Visualization Techniques"
  ];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      excerpt: "A comprehensive guide covering all the essential aspects of this topic with practical examples.",
      content: "Full article content would go here with detailed explanations, code examples, and illustrations...",
...
const generateTasks = (count: number, projectCount: number, assigneeCount: number): Record<string, unknown>[] => {
  const titles = [
    "Design new landing page", "Fix login bug", "Write documentation", "Code review PR #123",
    "Update dependencies", "Set up CI/CD pipeline", "Create database schema", "Implement user authentication",
    "Optimize query performance", "Write unit tests", "Deploy to production", "Monitor server logs",
    "Research new features", "Create wireframes", "User testing session", "Fix accessibility issues"
  ];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const dueDate = Math.random() > 0.3 
      ...
      : null;
    const completedAt = status === "done" ? new Date(Date.now() - Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000).toISOString() : null;
...
const generateProjects = (count: number, ownerCount: number): Record<string, unknown>[] => {
  const names = [
    "Website Redesign", "Mobile App Launch", "API Integration", "E-commerce Platform",
    "Marketing Campaign", "Data Migration", "Security Audit", "Performance Optimization"
  ];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      end_date: new Date(startDate.getTime() + durationDays * 24 * 60 * 60 * 1000).toISOString(),
...
export const MOCK_TABLE_DATA: Record<string, Record<string, unknown>[]> = {
  ...
  "ecommerce_db.public.categories": [
    { id: 1, name: "Electronics", slug: "electronics", description: "Electronic devices and accessories", parent_id: null, sort_order: 1 },
    { id: 2, name: "Clothing", slug: "clothing", description: "Apparel and fashion items", parent_id: null, sort_order: 2 },
    { id: 3, name: "Home & Garden", slug: "home-garden", description: "Home improvement and garden supplies", parent_id: null, sort_order: 3 },
    { id: 4, name: "Sports", slug: "sports", description: "Sports equipment and activewear", parent_id: null, sort_order: 4 },
    { id: 5, name: "Books", slug: "books", description: "Books and educational materials", parent_id: null, sort_order: 5 },
    { id: 6, name: "Toys", slug: "toys", description: "Toys and games for all ages", parent_id: null, sort_order: 6 },
    { id: 7, name: "Food & Beverages", slug: "food-beverages", description: "Food and drink products", parent_id: null, sort_order: 7 },
    { id: 8, name: "Beauty", slug: "beauty", description: "Beauty and personal care products", parent_id: null, sort_order: 8 },
  ],
  ...
  "user_service.main.users": Array.from({ length: 5000 }, (_, i) => ({
    ...
    created_at: new Date(2020 + Math.floor(Math.random() * 5), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)).toISOString(),
    updated_at: new Date(2024, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)).toISOString(),
  })),
