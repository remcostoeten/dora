            ...
            {DATABASE_TYPES.map(function (type) {
                ...
                return (
                    <button
                        key={type}
                        type="button"
                        onClick={function () { if (!isTypeDisabled) onSelect(type); }}
                        disabled={disabled || isTypeDisabled}
                        className={cn(
                            "db-card text-left",
                            isActive && "active",
                            (disabled || isTypeDisabled) && "opacity-50 cursor-not-allowed hover:bg-card/50 hover:border-border"
                        )}
                    >
                        <div className="flex items-center gap-3">
                            <div className={cn("db-card-icon bg-muted/50", (disabled || isTypeDisabled) && "grayscale")}>
