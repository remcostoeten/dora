            ...
            <div className="flex items-center justify-between h-8 px-2 border-b border-sidebar-border bg-sidebar-accent/30 shrink-0">
                ...
                {result && !result.error && (
                    <span className="text-xs text-muted-foreground">
                        {filteredRows.length !== result.rowCount ? `${filteredRows.length} / ` : ""}{result.rowCount} row{result.rowCount !== 1 ? "s" : ""} • {result.executionTime}ms backend • {filterTime}ms filtering
                    </span>
                )}
                                ...
                                <tbody>
                                    {filteredRows.map((row, rowIndex) => (
                                        <ContextMenu key={rowIndex}>
                                                    ...
                                                    {result.columns.map((col) => {
                                                        ...
                                                        return (
                                                            <td
                                                                ...
                                                                onDoubleClick={() => handleCellDoubleClick(rowIndex, col, cellValue)}
                                                            >
                                                                {isEditing ? (
                                                                    <Input
                                                                        ref={inputRef}
                                                                    ...
                                                                    <div className="truncate max-w-[300px]" title={String(cellValue)}>
                                            ...
                                            <ContextMenuContent>
                                                <ContextMenuItem onClick={() => navigator.clipboard.writeText(JSON.stringify(row, null, 2))}>
