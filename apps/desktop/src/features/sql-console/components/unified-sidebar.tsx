...
import {
    Search, Table2, Eye, ChevronRight, ChevronDown, Key, Hash, Type, Calendar,
    ToggleLeft, Copy, Database, FileText, FolderPlus, FilePlus, FileCode,
    Folder, Edit2, Trash2, Wand2
} from "lucide-react";
...
function ColumnIcon({ type, isPrimaryKey }: { type: string; isPrimaryKey?: boolean }) {
    ...
    if (type.includes("int") || type.includes("serial") || type.includes("decimal")) return <Hash className="h-3 w-3 text-blue-400" />;
    if (type.includes("char") || type.includes("text")) return <Type className="h-3 w-3 text-green-400" />;
    if (type.includes("date") || type.includes("time")) return <Calendar className="h-3 w-3 text-orange-400" />;
...
export function UnifiedSidebar({
    ...
    const toggleFolder = function (id: string) {
        setExpandedFolders(function (prev) { return { ...prev, [id]: !prev[id] }; });
    };
    ...
    const snippetTree = useMemo(function () {
        function buildTree(parentId: string | null): SqlSnippet[] {
            return snippets
                .filter(function (s) { return s.parentId === parentId; })
                .sort(function (a, b) {
    ...
    function renderSnippetNode(node: SqlSnippet, depth: number) {
        ...
        const children = snippets.filter(function (s) { return s.parentId === node.id; });
                ...
                <ContextMenuTrigger>
                    <div>
                        <button
                            ...
                            onClick={function () { node.isFolder ? toggleFolder(node.id) : onSnippetSelect(node.id); }}
                        >
                        ...
                        {node.isFolder && isExpanded && children.map(function (child) { return renderSnippetNode(child, depth + 1); })}
                    </div>
                </ContextMenuTrigger>
                <ContextMenuContent>
                    ...
                    <ContextMenuSeparator />
                    <ContextMenuItem className="text-destructive focus:text-destructive" onClick={function () { onDeleteSnippet(node.id); }}>
    ...
    return (
        <div className="flex flex-col h-full w-full border-r border-sidebar-border bg-sidebar overflow-hidden">
            <div className="flex border-b border-sidebar-border shrink-0">
                <button
                    onClick={function () { setActiveTab("schema"); }}
                    className={cn(
                ...
                <button
                    onClick={function () { setActiveTab("snippets"); }}
                    className={cn(
                        "flex-1 px-3 py-2 text-xs font-medium transition-all border-b-2",
                        activeTab === "snippets"
                            ? "border-primary text-foreground"
                    ...
                    <div className="p-2 border-b border-sidebar-border shrink-0">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                value={schemaSearch}
                                onChange={function (e) { setSchemaSearch(e.target.value); }}
                                        ...
                                        <ContextMenu>
                                            <ContextMenuTrigger>
                                                <div className="flex items-center group w-full px-2 py-1 hover:bg-sidebar-accent/50 transition-colors">
                                                    <button
                                                        onClick={function () { toggleTable(table.name); }}
                                                        className={cn(
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={function () { onTableSelect?.(table.name); }}
                                                    >
                                                        {table.type === "view" ? (
                                            ...
                                            <ContextMenuContent>
                                                <ContextMenuSub>
                                                    ...
                                                    <ContextMenuSubContent>
                                                        <ContextMenuItem onClick={function () { onInsertQuery?.(`SELECT * FROM ${table.name} LIMIT 100;`); }}>
                                                            <Database className="h-3.5 w-3.5" />
                                                            SELECT * FROM {table.name}
                                                        </ContextMenuItem>
                                                        <ContextMenuItem onClick={function () { onInsertQuery?.(`SELECT COUNT(*) FROM ${table.name};`); }}>
                                                            <FileText className="h-3.5 w-3.5" />
                                                            COUNT rows
                                                        </ContextMenuItem>
                                                        <ContextMenuItem onClick={function () { onInsertQuery?.(`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '${table.name}';`); }}>
                                                            <FileText className="h-3.5 w-3.5" />
                                                            DESCRIBE table
                                                        </ContextMenuItem>
                                                    </ContextMenuSubContent>
                                                ...
                                                <ContextMenuSeparator />
                                                <ContextMenuItem onClick={function () { copyToClipboard(table.name); }}>
                                                    ...
                                                    return (
                                                        <ContextMenu key={`${table.name}-${col.name}`}>
                                                            <ContextMenuTrigger>
                                                                ...
                                                                >
                                                                    <ColumnIcon type={col.type} isPrimaryKey={col.primaryKey} />
                                                                    <span className={cn("truncate", col.primaryKey && "font-medium text-foreground")}>{col.name}</span>
                                                                    <span className="text-[10px] text-muted-foreground/50 ml-auto font-mono">{col.type}</span>
                                                                </div>
                                                            </ContextMenuTrigger>
                                                            <ContextMenuContent>
                                                                <ContextMenuItem onClick={function () { onInsertQuery?.(col.name); }}>
                                                                    <FileText className="h-3.5 w-3.5" />
                                                                    Insert column name
                                                                </ContextMenuItem>
                                                                <ContextMenuItem onClick={function () { onInsertQuery?.(`SELECT ${col.name} FROM ${table.name} LIMIT 100;`); }}>
                                                                    <Database className="h-3.5 w-3.5" />
                                                                    SELECT {col.name}
                                                                </ContextMenuItem>
                                                                <ContextMenuSeparator />
                                                                <ContextMenuItem onClick={function () { copyToClipboard(col.name); }}>
                                                                    <Copy className="h-3.5 w-3.5" />
                                                                    Copy column name
                                                                </ContextMenuItem>
                                                            </ContextMenuContent>
            ...
            {activeTab === "snippets" && (
                <>
                    <div className="flex items-center justify-between gap-1 p-2 border-b border-sidebar-border shrink-0">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/50 px-1">Library</span>
                        <div className="flex items-center gap-0.5">
                            <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-sidebar-foreground" onClick={function () { onNewFolder(null); }} title="New folder">
                                <FolderPlus className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-sidebar-foreground" onClick={function () { onNewSnippet(null); }} title="New snippet">
                                <FilePlus className="h-3.5 w-3.5" />
                            </Button>
                        </div>
                    </div>
                    <div className="p-2 border-b border-sidebar-border shrink-0">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                className="h-7 bg-transparent border-sidebar-border/60 text-xs pl-7"
                                value={snippetSearch}
                                onChange={function (e) { setSnippetSearch(e.target.value); }}
                            />
                        </div>
                    </div>
                    <ScrollArea className="flex-1">
                        <div className="py-2">
                            {snippetTree.map(function (node) { return renderSnippetNode(node, 0); })}
                        </div>
                    </ScrollArea>
                </>
            )}
