...
export function QuerySidebar({
	...
	function startRenaming(node: SqlSnippet) {
	...
	const tree = useMemo(() => {
		function buildTree(parentId: string | null = null): SqlSnippet[] {
			return snippets
	...
	function renderNode(node: SqlSnippet, depth: number = 0) {
				...
				<ContextMenuTrigger>
					<div key={node.id}>
						<button
							...
							onClick={() =>
								node.isFolder
									? toggleFolder(node.id, {} as any)
									: onSnippetSelect(node.id)
							}
						>
							{NodeContent}
							{!isEditing && (
