...
export function DateCell({ value, columnName }: Props) {
    ...
    return (
        <div className="group flex items-center gap-2 min-w-0" title={`${fullDate} ${time} (${relativeTime})`}>
            {isExpiringColumn && (
                <div
                    className={cn(
            ...
            <div className="flex items-baseline gap-1.5 truncate font-mono text-xs">
                <span className={cn("text-foreground", isExpiringColumn && isPastDate && "text-muted-foreground line-through decoration-muted-foreground/50")}>
                    {fullDate}
                </span>
