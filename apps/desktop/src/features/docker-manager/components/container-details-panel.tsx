...
export function ContainerDetailsPanel({
    ...
    const { data: logs, isLoading: logsLoading, refetch: refetchLogs } = useContainerLogs(
        container?.id ?? null,
        { tail: tailLines, enabled: activeTab === "logs" }
    );
            ...
            <div className="flex-1 flex items-center justify-center p-8 border-l border-border">
                <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground max-w-[200px]">
                        Click on a container from the list to view its details and available actions.
                    </p>
                </div>
            </div>
        );
            ...
            <div className="flex-1 flex flex-col min-h-0 p-4">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
                    <TabsList className="w-full">
                        ...
                        <TabsTrigger value="seed" className="flex-1">Seed</TabsTrigger>
                    </TabsList>
