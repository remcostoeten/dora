...
export function isLocalHost(host: string): boolean {
    ...
    if (LOCAL_HOST_PATTERNS.some(function (pattern) {
        return normalizedHost === pattern;
    })) {
        return true;
    }

    if (LOCAL_IP_REGEX_PATTERNS.some(function (pattern) {
...
export function validateDestructiveAction(
    ...
    if (container.state === "running" && action === "remove") {
        return {
            safe: true,
            warning: "This will stop and remove the running container. Any unsaved work will be lost.",
        };
    }
