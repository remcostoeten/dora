...
function getEnvRecordingMode(): boolean {
    if (typeof import.meta !== "undefined" && import.meta.env) {
        return import.meta.env.VITE_RECORDING_MODE === "true" || import.meta.env.VITE_RECORDING_MODE === "1";
    }
    return false;
}
...
export function RecordingProvider({ children, forceEnabled, initialConfig }: Props) {
    ...
    useEffect(function applyZoom() {
        if (isRecordingMode && config.focusZoom !== 1) {
            document.documentElement.style.setProperty("--recording-zoom", String(config.focusZoom));
    ...
    useEffect(function applyCustomCss() {
    ...
    const value = useMemo<RecordingContextType>(function createValue() {
        return {
            isRecordingMode,
            config,
            updateConfig,
...
export function useRecording(): RecordingContextType {
    ...
    if (!ctx) {
        return {
            ...
            shouldHide: function noop() { return false; },
        };
    }
    return ctx;
}
