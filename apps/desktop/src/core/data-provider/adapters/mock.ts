...
export function createMockAdapter(): DataAdapter {
    return {
        ...
        async updateConnection(id: string, name: string, databaseType: DatabaseInfo, sshConfig: JsonValue | null): Promise<AdapterResult<ConnectionInfo>> {
            await randomDelay();
            const idx = store.connections.findIndex(function (c) { return c.id === id; });
            if (idx === -1) {
        ...
        async connectToDatabase(connectionId: string): Promise<AdapterResult<boolean>> {
            await randomDelay();
            const conn = MOCK_CONNECTIONS.find(function (c) { return c.id === connectionId; });
            if (!conn) {
                return err("Connection not found");
            }
            return ok(true);
        ...
        async fetchTableData(
            ...
            const schema = MOCK_SCHEMAS[connectionId];
            const tableInfo = schema?.tables.find(function (t) { return t.name === tableName; });
