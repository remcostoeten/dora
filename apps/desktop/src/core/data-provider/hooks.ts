...
export function useConnectionMutations() {
    ...
    const addConnection = useMutation({
        mutationFn: async (params: { name: string; databaseType: any; sshConfig: any }) => {
            const res = await adapter.addConnection(params.name, params.databaseType, params.sshConfig);
            if (!res.ok) throw new Error(res.error);
            return res.data;
        },
        onSuccess: () => {
    ...
    const updateConnection = useMutation({
        mutationFn: async (params: { id: string; name: string; databaseType: any; sshConfig: any }) => {
            const res = await adapter.updateConnection(params.id, params.name, params.databaseType, params.sshConfig);
            if (!res.ok) throw new Error(res.error);
            return res.data;
        },
        onSuccess: () => {
...
export function useTableData(
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await adapter.fetchTableData(connectionId, tableName, page, pageSize, sort, filters);
            if (!res.ok) throw new Error(res.error);
            return res.data;
        },
