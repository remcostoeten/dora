	...
	for (let i = 0; i < AUDIT_LOG_COUNT; i++) {
		logs.push({
			...
			changes: JSON.stringify({
				field: randomFrom(['name', 'status', 'price', 'quantity']),
				old: 'value1',
				new: 'value2'
			}),
...
function generateApiLogs(): Record<string, unknown>[] {
	...
	for (let i = 0; i < API_LOGS_COUNT; i++) {
		const statusCode =
			Math.random() > 0.1
				? randomFrom([200, 201, 204])
				: randomFrom([400, 401, 403, 404, 500])
		logs.push({
			id: i + 1,
			...
			error_message:
				statusCode >= 400
					? randomFrom(['Invalid request', 'Unauthorized', 'Not found', 'Internal error'])
					: null,
