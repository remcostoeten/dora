...
type TRailItem = { icon: LucideIcon; label: string; view?: TDemoView }
...
const railItems: TRailItem[] = [
    { icon: SquareTerminal, label: 'SQL Console', view: 'sql' },
    { icon: Table2, label: 'Data Viewer', view: 'data' },
    { icon: ChartLine, label: 'Analytics' },
    { icon: Network, label: 'Schema', view: 'schema' },
            ...
            <aside className="relative z-0 flex h-full w-16 shrink-0 flex-col bg-sidebar border-r border-sidebar-border">
                ...
                <nav className="flex w-full flex-1 flex-col gap-1 p-2">
                    <div className="mx-auto flex flex-col gap-1">
                        {railItems.map(function renderRail(item) {
                            return (
                                <RailButton
                                    key={item.label}
                                    icon={item.icon}
