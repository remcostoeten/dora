...
function GridCheckbox({
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className="flex items-center justify-center border-b border-r border-l border-sidebar-border bg-background transition-colors hover:bg-sidebar-accent/50"
        >
            <span
                className={
    ...
    function renderCell(cell: string, cellIndex: number) {
        ...
        const tone =
            cellIndex === 0
                ...
                : 'text-foreground/90'
        const focus = focused
            ? ' relative ring-1 ring-inset ring-primary/70'
            : ''
        return (
            <button
                type="button"
...
function GhostButton({
    ...
    return (
        ...
        >
            <Icon
                className={
                    'h-3.5 w-3.5 shrink-0 opacity-90 ' + (iconClass ?? '')
                }
            />
            <span>{label}</span>
        </button>
...
export function DemoMain({ activeTable }: { activeTable: string }) {
    ...
    const [deletingRows, setDeletingRows] = useState<Map<string, number>>(new Map())
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
            ...
            <div className="flex items-center h-9 pr-2 gap-1.5 text-sm bg-sidebar border-b border-sidebar-border shrink-0">
                ...
                <div className="flex items-center gap-1">
                    <GhostButton
                        icon={Edit3}
                        label={dryEdit ? 'Editing' : 'Dry Edit'}
                        active={dryEdit}
            ...
            <div className="flex items-center justify-between h-10 px-3 bg-sidebar border-t border-sidebar-border shrink-0">
                {selectedRows.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        ...
                        <span>
                            Showing 1-{displayRows.length} of{' '}
            ...
            <button
                type="button"
                className="absolute bottom-4 right-4 z-10 flex h-10 w-10 items-center justify-center rounded-[3px] border border-border bg-background shadow-lg transition-colors hover:bg-sidebar-accent"
            >
