...
const CUSTOMERS: TDemoTable = {
    ...
    columns: [
        { name: 'id', type: 'serial', kind: 'pk', track: '52px' },
        {
            ...
            type: 'varchar(100)',
            kind: 'text',
            track: 'minmax(70px, 1.1fr)'
        },
        {
            name: 'email',
            type: 'varchar(255)',
            kind: 'text',
            track: 'minmax(70px, 1.4fr)'
        },
        {
            name: 'phone',
            type: 'varchar(20)',
            kind: 'text',
            track: 'minmax(70px, 0.9fr)'
        },
        {
            name: 'city',
            type: 'varchar(50)',
            kind: 'text',
            track: 'minmax(70px, 0.7fr)'
        },
        {
            name: 'country',
            type: 'varchar(50)',
            kind: 'text',
            track: 'minmax(70px, 0.8fr)'
        },
        {
            name: 'created_at',
            type: 'timestamp',
...
const connectionDatasetProfiles: Record<
    string,
    { rowRotation: number; idOffset: number }
> = {
    demo: { rowRotation: 0, idOffset: 0 },
    prod: { rowRotation: 2, idOffset: 1000 },
    analytics: { rowRotation: 4, idOffset: 2000 },
    mariadb: { rowRotation: 6, idOffset: 3000 },
    staging: { rowRotation: 8, idOffset: 4000 },
    cockroach: { rowRotation: 10, idOffset: 5000 },
    local: { rowRotation: 1, idOffset: 6000 }
}

function offsetId(value: string, offset: number): string {
    const numeric = Number(value)
    return value && Number.isFinite(numeric) ? String(numeric + offset) : value
}

/**
 * Materialize the same demo schema as a separate database dataset. Rotating
 * the record values keeps every connection recognizable while the key offset
 * makes it immediately clear that switching connections loaded different
 * rows rather than merely relabeling the active tab.
 */
function tableForConnection(
    table: TDemoTable,
    connectionId: string
): TDemoTable {
    const profile =
        connectionDatasetProfiles[connectionId] ??
        connectionDatasetProfiles.demo
    if (profile.rowRotation === 0 && profile.idOffset === 0) return table

    const rows = table.rows.map((row, rowIndex) => {
        const source =
            table.rows[(rowIndex + profile.rowRotation) % table.rows.length]

        return table.columns.map((column, columnIndex) => {
            if (column.kind === 'pk') {
                return offsetId(row[columnIndex], profile.idOffset)
            }
            if (column.kind === 'fk') {
                return offsetId(source[columnIndex], profile.idOffset)
            }
...
export function findTable(
    name: string,
    connectionId: string = 'demo'
): TDemoTable {
    const table = demoTables.find((entry) => entry.name === name) ?? CUSTOMERS
    return tableForConnection(table, connectionId)
}
