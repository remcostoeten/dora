...
export function DrizzleRunnerCard({ animate }: { animate: boolean }) {
    ...
    // A clean type-in from empty the moment the card scrolls into view, no flash
    // of a completed query that then wipes itself, which is what made the entrance
    // feel janky on first load.
    const shownChars = running ? revealed : 0
    const shownRows = running ? rows : 0
    const langAccent =
        activeLang === 'prisma'
            ? 'var(--color-vendor-prisma)'
            : 'var(--color-brand-300)'
    ...
    function status(): { color: string; label: string } {
        ...
        if (isType)
            return { color: 'var(--color-ink-600)', label: 'autocomplete' }
        return {
            color: 'var(--color-status-ok-dim)',
            label: `${shownRows} rows · 3 ms`
        }
    }
        ...
        >
                    ...
                    {ROWS.map((row, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className="text-right text-brand-300">
                                    {row.plan}
                                </span>
                            </div>
                        )
            ...
            <div className="relative px-5 pt-3 pb-10">
                ...
                <p className="text-xs text-ink-500 leading-relaxed">
                    Prefer an ORM over raw SQL? Run type-safe Drizzle or Prisma
                    Client queries: context-aware autocomplete and a live SQL
                    preview, right in the query builder.
                </p>
            </div>
        </div>
    )
