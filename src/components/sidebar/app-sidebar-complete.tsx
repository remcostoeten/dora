...
export function AppSidebarComplete({
  ...
  // Common small sidebar content to avoid duplication in motion variants if needed, 
  // but for smooth transition we render the full structure and hide parts.
  // Actually, for a "snap" animation, we want the container to animate width, 
  // and the content to cross-fade or slide.

  return (
    <div className="flex h-full flex-col border-r border-sidebar-border bg-card overflow-hidden">
      {/* Small Sidebar Content (Overlay/Replacement) */}
      <div
        className={`absolute inset-0 flex flex-col bg-card transition-opacity duration-300 ${isSidebarCollapsed ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      >
      ...
      <div
        className={`flex h-full flex-col transition-opacity duration-300 ${!isSidebarCollapsed ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      >
